...
const [items, setItems] = useState([])
  ...
  const processSheetData = (sheetData) => {
    ...
    actualDataRows.forEach((row) => {
      ...
      // Column AB (index 27) - Item name
      if (row[27] && row[27].toString().trim() !== "") {
        itemsSet.add(row[27].toString().trim())
      }
    })
    ...
    processedData.items = Array.from(itemsSet).sort()
...
const isDateInRange = (dateStr, fromDate, toDate) => {
  ...
  }, [data?.rawData, fromDate, selectedItem, selectedParty, selectedSalesperson, selectedState, toDate])

  // Calculate metrics based on filtered data
const calculateFilteredMetrics = (rows) => {
  ...
  const displayMetrics = useMemo(() => calculateFilteredMetrics(filteredData), [filteredData])
  ...
  const chartData = useMemo(() => {
    ...
    const chartDataSet = top10.map(([name, value], index) => ({
      name,
      value,
