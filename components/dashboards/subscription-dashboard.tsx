...
export default function SubscriptionDashboard() {
  const [sheets, setSheets] = useState<DashboardResponse>({
    subscriptionSheet: [],
    renewalSheet: [],
  });
    ...
    const load = async () => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          data = Array.isArray(incoming)
            ? { subscriptionSheet: incoming, renewalSheet: [] }
            : incoming;
        }
        setSheets({
          subscriptionSheet: Array.isArray(data.subscriptionSheet)
            ? data.subscriptionSheet
            : [],
          renewalSheet: Array.isArray(data.renewalSheet)
            ? data.renewalSheet
            : [],
        });
      } catch (err) {
  ...
  const stats = useMemo(() => {
    ...
    const pendingApprovals = filteredSubscriptions.filter(
      (row) =>
        (row.actual2 ?? row.actual_2 ?? "") === "" &&
        (row.planned2 ?? row.planned_2)
    ).length;
    const pendingPayments = filteredSubscriptions.filter(
      (row) =>
        (row.actual3 ?? row.actual_3 ?? "") === "" &&
        (row.planned3 ?? row.planned_3)
    ).length;
    const pendingRenewals = filteredSubscriptions.filter(
      (row) =>
        (row.actual1 ?? row.actual_1 ?? "") === "" &&
        (row.planned1 ?? row.planned_1)
    ).length;

    const totalValue = filteredSubscriptions.reduce((acc, row) => {
      const renewalCount = Number.isNaN(
        Number.parseInt(row.renewalCount || "", 10)
      )
        ? 1
    ...
    const totalRenewalValue = (sheets.renewalSheet || []).reduce(
      (acc, row) => acc + toNumber(row.price),
      0
    );
  ...
  const monthlySeries = useMemo(() => {
    const monthMap: Record<
      string,
      { month: string; active: number; revenue: number }
    > = {};
    ...
    filteredSubscriptions.forEach((row) => {
      ...
      if (!monthMap[monthLabel])
        monthMap[monthLabel] = { month: monthLabel, active: 0, revenue: 0 };
    ...
    return series.length
      ...
      : monthOrder
          .slice(0, 6)
          .map((m) => ({ month: m, active: 0, revenue: 0 }));
  }, [filteredSubscriptions]);
        ...
        <div
          onClick={() =>
            handleStatCardClick(
              ...
              filteredSubscriptions.filter((row) => getStatus(row) === "Active")
            )
          }
          className="cursor-pointer"
        >
          ...
          <CardContent className="flex gap-3 items-center p-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {stats.pendingApprovals || "No"} Subscription
        ...
        <Card className="border-0 border-l-4 border-green-500 rounded-md bg-white/90">
          <CardContent className="flex gap-3 items-center p-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {stats.pendingPayments || "No"} Subscription
        ...
        <Card className="border-0 border-l-4 border-blue-500 rounded-md bg-white/90">
          <CardContent className="flex gap-3 items-center p-4">
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {stats.pendingRenewals || "No"} Subscription
                {stats.pendingRenewals !== 1 && "s"}
              </p>
            </div>
          </CardContent>
        </Card>
                ...
                {costlySubscriptions.map((sub, idx) => (
                  <tr key={sub.subscriptionNo || idx} className="border-t">
                    ...
                    <td className="py-2 text-right">
                      {currencyFormatter(toNumber(sub.price))}
                    </td>
                  </tr>
                ))}
